...
import { Formik, Field as FormikField, FieldProps as FormikFieldProps, Form as FormikForm, FormikHelpers } from 'formik';
...
export const Field = forwardRef<HTMLInputElement, FieldProps>(({ id, name, label, showLabel, description, ...props }, ref) => {
	return (
			...
			{({ field, form: { errors, touched } }: FormikFieldProps) => (
				<div>
					...
					<div css={[ tw`relative`, showLabel && tw`mt-1` ]}>
						<input
							id={id}
							css={[ tw`block w-full px-3 py-2 placeholder-gray-400 border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none focus:ring-rose-500 focus:border-rose-500 sm:text-sm`, (touched[field.name] && errors[field.name]) && tw`text-red-900 placeholder-red-300 border-red-300` ]}
							{...field}
							{...props}
						/>
					...
					{touched[field.name] && errors[field.name] ?
						...
						:
						description ? <p id={id + '-description'} css={tw`mt-2 text-sm text-gray-500`}>{description}</p> : null
					}
				</div>
			)}
		</FormikField>
